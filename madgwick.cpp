...
MadgwickFilter::MadgwickFilter(const float sampleFrequency, const float beta_coeff)
{
	...
	outputQuaternion.x() = 0.0f;
	outputQuaternion.y() = 0.0f;
...
void MadgwickFilter::getEulerDeg(Eigen::Vector3f& eulerDeg)
{	
	eulerDeg(0) = asinf(-2.0f * (q1*q3 - q0*q2)) * 57.29578f;							//Pitch
	eulerDeg(1) = atan2f(q0*q1 + q2*q3, 0.5f - q1*q1 - q2*q2) * 57.29578f;				//Roll
	eulerDeg(2) = atan2f(q1*q2 + q0*q3, 0.5f - q2*q2 - q3*q3) * 57.29578f + 180.0f;		//Yaw
}	

void MadgwickFilter::getQuaternion(Eigen::Quaternion<float>& quaternion)
{
...
void MadgwickFilter::update9DOF(Eigen::Vector3f accelerometer, Eigen::Vector3f gyroscope, Eigen::Vector3f magnetometer)
{
	...
	*---------------------------------*/
	ax = accelerometer(X); gx = gyroscope(X); mx = magnetometer(X);
	ay = accelerometer(Y); gy = gyroscope(Y); my = magnetometer(Y);
	az = accelerometer(Z); gz = gyroscope(Z); mz = magnetometer(Z);
	...
	if(!((ax == 0.0f) && (ay == 0.0f) && (az == 0.0f))) {
		...
		/*----------------------------------
		* Normalize measurements and assign them to variables 
		*---------------------------------*/
	...
	outputQuaternion.w() = q0;
	outputQuaternion.x() = q1;
	outputQuaternion.y() = q2;
	outputQuaternion.z() = q3;
}


// Fast Inverse Square Root (optimized)
